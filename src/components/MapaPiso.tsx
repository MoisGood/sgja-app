                        ...
                        return (
                          <div key={key} style={{ borderRadius: 6, border: '1px solid #e5e7eb', overflow: 'hidden' }}>
                            ...
                            >
                              ...
                              <button
                                onClick={async e => { e.stopPropagation(); if (!ui.selected) return; const u = ubicaciones.find(x => x.dispositivo_nombre === key); if (groupEquipos.length === 0 && !u) return; if (groupEquipos.length > 0 && !confirm(`Hay ${groupEquipos.length} equipo(s) asignado(s) a "${key}". ¿Borrar todo?`)) return; if (groupEquipos.length > 0) { const ids = new Set(groupEquipos.map(e => e.id)); setEquipos(prev => prev.filter(e => !ids.has(e.id))); } await Promise.all(groupEquipos.map(eq => supabase.from('equipos').update({ activo: false }).eq('id', eq.id).eq('id_establecimiento', idEstablecimiento))); if (u) await supabase.from('ubicaciones').update({ activo: false }).eq('id', u.id).eq('id_establecimiento', idEstablecimiento); await cargarUbicaciones(ui.selected.id); }}
                                ...
                                {groupEquipos.map(eq => (
                                  <div key={eq.id} style={{
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                      ...
                                      <button
                                        onClick={async e => { e.stopPropagation(); if (!ui.selected) return; if (!confirm(`¿Quitar "${eq.nombre}" de ${ui.selected.nombre}?`)) return; setEquipos(prev => prev.filter(e => e.id !== eq.id)); await supabase.from('equipos').update({ activo: false }).eq('id', eq.id).eq('id_establecimiento', idEstablecimiento); const ubic = ubicaciones.find(u => u.dispositivo_nombre === key); if (ubic) { if (ubic.cantidad <= 1) { await supabase.from('ubicaciones').update({ activo: false }).eq('id', ubic.id).eq('id_establecimiento', idEstablecimiento); } else { await supabase.from('ubicaciones').update({ cantidad: ubic.cantidad - 1 }).eq('id', ubic.id).eq('id_establecimiento', idEstablecimiento); } } await cargarUbicaciones(ui.selected.id); }}
